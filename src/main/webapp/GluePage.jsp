...
<%@ page import="java.util.ArrayList, javax.servlet.http.HttpServletRequest, model.Product,
 model.Catalog, javax.servlet.http.HttpSession,
 model.User,
 model.Supplies"%>
<!DOCTYPE html>
<html>
...
<link href="css_styles/Glue.css" rel="stylesheet">
...
<% 
   ...
   ArrayList<Product> glueProducts=catalog.getGlueProducts();
   request.setAttribute("GlueProducts",glueProducts);
%>
	...
	<div class="Glue">
		<div class="stroka">
			...
			<label for="layers">Объем</label>
			<label for="price">Цена</label>
			<label for="stock">Остаток</label>
		</div>

	
	  <c:if test="${GlueProducts.size()!=0}">
	  <div class="row">
	  <c:forEach var="product" items="${GlueProducts}">
		<tr>
		
		<form method="post" action="EditGlue">
		
			<label >${product.getName()}</label>
			<input type="text" name="type" value="${product.getTypeName()}">
			<input type="number" step="any" name="volume" value="${product.getOtherParameters()[0]}">
	...
	<form method="post" action="AddGlue">		 
	  <div class="str1">
	  	<input type="text" class="form-control" name="productType" placeholder="Тип">
	  	<input type="number" step="any" class="form-control" name="volume" placeholder="Объем"> 
	  	<input type="number" step="any" class="form-control" name="cost" placeholder="Цена">
	  	<input type="number" class="form-control" name="number" placeholder="Остаток на складе">
